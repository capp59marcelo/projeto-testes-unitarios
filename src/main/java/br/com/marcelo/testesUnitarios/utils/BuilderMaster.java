package br.com.marcelo.testesUnitarios.utils;
...
public class BuilderMaster
{
	...
	public void gerarCodigoClasse(Class classe)
	{
		...
		builder.append("\tpublic static ").append(nomeClasse).append(" um").append(classe.getSimpleName())
				.append("() {\n");
		...
		for(Field campo : declaredFields)
		{
			...
			builder.append("\t\telemento.set").append(campo.getName().substring(0, 1).toUpperCase())
					.append(campo.getName().substring(1)).append("(").append(getDefaultParameter(campo)).append(");\n");
		...
		for(Field campo : declaredFields)
		{
			...
			if(campo.getType().getSimpleName().equals("List"))
			{
				...
				builder.append("\tpublic ").append(nomeClasse).append(" comLista")
						.append(campo.getName().substring(0, 1).toUpperCase()).append(campo.getName().substring(1))
						.append("(").append(((Class) stringListType.getActualTypeArguments()[0]).getSimpleName())
						.append("... params) {\n");
				builder.append("\t\telemento.set").append(campo.getName().substring(0, 1).toUpperCase())
						.append(campo.getName().substring(1)).append("(Arrays.asList(params));\n");
